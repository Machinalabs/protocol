...
const argv = require("minimist")(process.argv.slice(), {
  string: ["poolAddress", "empAddress", "tokenName", "sushiPoolAddress"],
...
async function calculateUniswapLPRewards(
  ...
  const sushiSwapPoolInfo = await _fetchSushiswapPoolInfo(sushiPoolAddress);
  console.log("sushiSwapPoolInfo", JSON.stringify(sushiSwapPoolInfo));

  const sushiShareHolders = sushiSwapPoolInfo.flatMap(a => a.user.id);
  console.log("sushiShareHolders", sushiShareHolders);
  console.log("🏖  Number of historic liquidity providers in sushi:", sushiShareHolders.length);

  // Initialize the contract we'll need for computation.
  const uniswapPool = new web3.eth.Contract(getAbi("ERC20"), poolAddress);

  console.log("SUSHI sushiPoolAddress", sushiPoolAddress);
  const sushiswapPool = new web3.eth.Contract(getAbi("ERC20"), sushiPoolAddress);
...
async function _calculatePayoutsBetweenBlocks(
  ...
  for (let shareHolder of [...shareHolders, ...sushiShareHolders]) {
    shareHolderPayout[shareHolder] = toBN("0");
  }

  let uniswapShareHolderPayout = {};
  for (let shareHolder of shareHolders) {
    uniswapShareHolderPayout[shareHolder] = toBN("0");
  }

  let sushiswapShareHolderPayout = {};
  for (let shareHolder of sushiShareHolders) {
    sushiswapShareHolderPayout[shareHolder] = toBN("0");
  }
  ...
  for (let currentBlock = fromBlock; currentBlock < toBlock; currentBlock += blocksPerSnapshot) {
    shareHolderPayout = await _updatePayoutAtBlock(
      ...
      currentBlock,
      shareHolderPayout,
      uniswapShareHolderPayout,
      sushiswapShareHolderPayout,
      umaPerSnapshot
    );
...
async function _updatePayoutAtBlock(
  ...
  // then merge here to get all positions, done..
  const SUSHI_POOL_BLOCK_CREATION = 13003574;
  ...
  if (blockNumber >= SUSHI_POOL_BLOCK_CREATION) {
    ...
    // Get the total number of synthetics in the Sushiswap pool at the snapshot's block number.
    syntheticsInPoolAtSnapshotInSushi = toBN(
      await syntheticToken.methods.balanceOf(sushiswapPool._address).call(undefined, blockNumber)
    );
  }
  ...
  const lpTokensToSynthetics = syntheticsInPoolAtSnapshot
    .add(syntheticsInPoolAtSnapshotInSushi)
    .mul(toBN(toWei("1")))
    .div(lpTokenSupplyAtSnapshot.add(lpTokenSupplyAtSnapshotInSushi));
...
// Find information about a given Sushiswap `poolAddress` `shares` returns a list of all historic LP providers.
async function _fetchSushiswapPoolInfo(poolAddress) {
  // https://gateway.thegraph.com/api/[api-key]/subgraphs/id/0x4bb4c1b0745ef7b4642feeccd0740dec417ca0a0-0
  const SUBGRAPH_URL = process.env.SUBGRAPH_URL || "https://api.thegraph.com/subgraphs/name/sushiswap/exchange";
  const query = `
  {
    liquidityPositions (where:{pair:"${poolAddress.toLowerCase()}"} ) {
      user {
  ...
  throw "⚠️  Sushiswap pool provided is not indexed in the subgraph or bad address!";
}
